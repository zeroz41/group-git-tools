...
# dry run?
if ! $dry_run; then
    read -p "This will push all repositories. Consider using -d for dry run first. Continue? [y/N] " confirm
    if [[ ! $confirm =~ ^[Yy]$ ]]; then
...
while read -r dir; do
    if [ -d "$dir/.git" ]; then
        ...
        # Push command
        push_command="git push $remote_name $branch"
        if ! exec_or_print "$push_command"; then
            echo -e "\e[31mFailed to push $dir. Continuing to next repository.\e[0m"
            failed_repos+=("$dir (push failed)")
            cd - >/dev/null || exit 1
            echo "------------------------"
            continue
        fi
        
        echo -e "\e[32mSuccessfully pushed $dir\e[0m"
        cd - >/dev/null || exit 1
        echo "------------------------"
    fi
...
if [ ${#failed_repos[@]} -eq 0 ]; then
    echo -e "\e[32m\e[1m\033#3All repositories were successfully pushed.\e[0m"
    echo ""
    echo ""
else
